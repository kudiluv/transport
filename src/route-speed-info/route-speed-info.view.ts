...
//             .createQueryBuilder()
//             .select('route.id', 'id')
//             .addSelect('ST_ClosestPoint(ST_POINTS(route.fullRoute), vehicle_position.position)', 'closest')
//             .addSelect('AVG(vehicle_position.speed)', 'avgSpeed')
//             .addSelect('COUNT(vehicle_position."position")', 'count')
//             .from('(select ST_LineMerge(ST_COLLECT(route."AB", route."BA")) as fullRoute, id from route)', 'route')
...
//             .andWhere('ST_Distance(ST_POINTS(route.fullRoute), vehicle_position.position) < 0.001')
//             .andWhere(
